...
*/
export const get_all_slots = async (
  req: Request,
  res: Response
) => {
  try {
...
export const create_slots = async (
  ...
  try {
    const slot: ISlot = new SlotsModel({
      slot_name: `${slot_name.split(' ').join('-')}-${new Date().toISOString().split('T')[0]}`,
    });
    await slot.save();
    return res.json({
      code: 200,
...
/* 
  ...
  @details: update slot
*/
export const update_slot = async (
  req: Request,
  res: Response
) => {
  const { slot_name } = req.body;
...
/* 
  ...
  @details: delete slot
*/
export const remove_slot = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    return res.json({
      code: 200,
      data: null,
      message: `${view_info.slot_name} ${successfully_deleted}`,
    });
  } catch (err) {
    return res.json({
      code: 400,
